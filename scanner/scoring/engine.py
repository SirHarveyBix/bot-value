...
from scanner.scoring.quality import calculate_quality_metrics, apply_quality_gates
from scanner.scoring.valuation import calculate_valuation_metrics, apply_valuation_gates
from scanner.scoring.momentum import calculate_momentum_metrics, apply_momentum_penalties, SECTOR_ETF_MAP
...
def stock_scoring_pipeline(all_data, symbols):
    ...
    df["rank_margin"] = df.groupby("sector")["margin"].transform(lambda x: compute_percentile_ranks(pd.DataFrame(x), "margin"))
    df["rank_debt"] = compute_percentile_ranks(df, "debt_ebitda", ascending=False)
    df["rank_fcf"] = compute_percentile_ranks(df, "fcf_yield")
    df["rank_pe"] = df.groupby("sector")["pe"].transform(lambda x: compute_percentile_ranks(pd.DataFrame(x), "pe", ascending=False))
    df["rank_ev_ebitda"] = df.groupby("sector")["ev_ebitda"].transform(lambda x: compute_percentile_ranks(pd.DataFrame(x), "ev_ebitda", ascending=False))
    ...
    df["score_valuation"] = (df["rank_pe"].fillna(0) * vw["pe_forward"] + df["rank_ev_ebitda"].fillna(0) * vw["ev_ebitda"] + 
                            df["rank_peg"].fillna(0) * vw["peg"])
    
    mw = CONFIG["scoring"]["momentum_subweights"]
    df["score_momentum"] = (df["rank_perf_6m"].fillna(0) * mw["perf_6m"] + df["rank_outperf_6m"].fillna(0) * mw["outperf_6m"] +
                           df["rank_perf_3m"].fillna(0) * mw["perf_3m"] + df["rank_eps_rev"].fillna(0) * mw["eps_revision"])
    
    df["score_momentum"] = df.apply(lambda r: apply_momentum_penalties(r["score_momentum"], r), axis=1)
    
    # Global
    w = CONFIG["scoring"]["weights"]
    df["score_global"] = df.apply(lambda r: (r["score_quality"] * w["quality"] + r["score_valuation"] * w["valuation"] + r["score_momentum"] * w["momentum"]) if r["v_ok"] 
                                 else (r["score_quality"] * 0.5 + r["score_momentum"] * 0.5), axis=1)
